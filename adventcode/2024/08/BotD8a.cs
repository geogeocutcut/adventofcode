            ...
            foreach (var a in Antennes)
            {
                foreach (var b in Antennes)
                {
                    if (a != b && a.Hz==b.Hz)
                    {
                        ...
                        if ((a.Position.X - vp.Item1) >= 0 && (a.Position.X - vp.Item1) < _map.GetLength(0)
                            && (a.Position.Y - vp.Item2) >= 0 && (a.Position.Y - vp.Item2) < _map.GetLength(1))
                            positionsPossibles.Add($"({a.Position.X - vp.Item1},{a.Position.Y - vp.Item2})");


                    }
                }
            }
            return positionsPossibles.Count(); ;
        }
